// This is a remote server hosted in node.

// It receives api calls, at http://localhost:8083/<some word>

// it checks to see if <some word> is in the word list, returning error case 'xxxxx' if not

// Then it returns a color code, of g's, y's, and r's, represent hit, near hit, and true miss values in wordle.

// "Return" here means sending json, this json is viewable in browser or can be read in a .js api call.

// Example: http://localhost:8083/asdfg -> {"data":"xxxxx"}
// Example: http://localhost:8083/pouty -> {"data":"grryr"} // assume word is 'pitch', which contains a p in the same place and t in a different place
...
server.on('request', (request, res) => {
	...
	res.end(JSON.stringify({data: color(request['url'])})) ;
}) ;

server.listen(8083) ;

// The color calculator
function color(iword) {
	// clip the slash
	iword = iword.slice(1) ;
	if (!words.includes(iword))
	{
		return "xxxxx" ; // Some error code I don't know.
	}
	// calc todays word rq
	const today = new Date() ;
	const index = Math.floor(today.getTime() / (1000 * 60 * 60 * 24)) ;
	const answr = commn[index % commn.length] ;
	let oword = '' ;
	for (let i = 0; i < 5; i++) {
		if (iword[i] == answr[i]) {
			oword  += 'g' ;
		} else if (answr.includes(iword[i])) {
			oword  += 'y' ;
			} else {
			oword  += 'r' ; // for red because I forgot what colors wordle used.
		}
	}
	return oword ;
}

// ask an LLM for ~500 common 5 letter words and remove things not in "WORDS"
// the size of this list is effectively the difficulty. small -> easy
const commn = [
    "about", "above", "abuse", "actor", "acute", "admit", "adopt", "adult", "after", "again",
    "agent", "agree", "ahead", "alarm", "album", "alert", "alike", "alive", "allow", "alone",
    "along", "alter", "among", "angle", "angry", "apart", "apple", "apply", "arena", "argue",
    "arise", "armed", "arrow", "aside", "asset", "audio", "audit", "avoid", "awake", "award",
    "aware", "basic", "basis", "beach", "begin", "being", "below", "bench", "birth", "black",
    "blame", "blind", "block", "blood", "board", "boost", "booth", "brain", "brake", "brand",
    "bread", "break", "breed", "brief", "bring", "broad", "brown", "brush", "build", "built",
    "burst", "buyer", "cable", "calms", "camel", "candy", "carry", "catch", "cause", "chain",
    "chair", "chart", "chase", "cheap", "check", "cheer", "child", "china", "chose", "civil",
    "claim", "class", "clean", "clear", "clerk", "click", "clock", "close", "cloud", "coach",
    "coast", "could", "count", "court", "cover", "craft", "crash", "cream", "crime", "cross",
    "crowd", "crown", "curve", "cycle", "daily", "dance", "death", "debut", "delay", "depth",
    "doing", "doubt", "dozen", "draft", "drama", "drawn", "dream", "dress", "drink", "drive",
    ...
    "grand", "grant", "grass", "great", "green", "greet", "guard", "guess", "guest", "guide",
    "happy", "harsh", "heart", "heavy", "hence", "horse", "hotel", "house", "human", "ideal",
    "image", "index", "inner", "input", "issue", "joint", "jolly", "judge", "juice", "knife",
    "knock", "known", "label", "large", "laser", "later", "laugh", "layer", "learn", "least",
    "leave", "legal", "level", "light", "limit", "local", "lucky", "lunch", "major", "maker",
    "march", "match", "maybe", "mayor", "means", "meant", "media", "metal", "might", "minor",
    ...
    "prior", "prize", "proof", "prove", "queen", "quick", "quiet", "quite", "radio", "raise",
    "range", "rapid", "ratio", "reach", "react", "ready", "refer", "right", "rival", "river",
    "robot", "rough", "round", "route", "royal", "rural", "scale", "scene", "scope", "score",
    "sense", "serve", "seven", "shall", "shape", "share", "sharp", "sheet", "shelf", "shell",
    "shift", "shirt", "shock", "shoot", "short", "sight", "since", "sixth", "sixty", "sized",
    ...
    "stuff", "style", "sugar", "suite", "super", "sweet", "table", "taken", "tarry", "taste",
    "teach", "teeth", "thank", "their", "theme", "there", "these", "thick", "thing", "think",
    "third", "those", "three", "throw", "tight", "tired", "title", "today", "topic", "total",
    "touch", "tough", "tower", "track", "trade", "train", "treat", "trend", "trial", "trust",
    "truth", "twice", "under", "union", "unite", "until", "upper", "upset", "urban", "usage",
...
// Allow players a broader dictionary than possible answers
const words = [
